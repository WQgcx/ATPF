...
def density(x,mean,variance):
    ...
    return torch.exp(val - val_max) + eps
...
C = 0
D = 1
tau = 0.36 # inflation coeff. of MPF
delta_reconx = 0.2 # inflaction coeff. of ATPF
lamda = 0.0015
...
M2 = 100000 # the number of PF samples
max_iter = 500
...
for t in range(T+1): # train transformation[t]
    ...
    optimizer = torch.optim.Adam(transformation[t].parameters(),lr=0.008)
    ...
    for iteration in range(1,max_iter+1):
        ...
        width = torch.median(torch.cat([distance1,distance2])) * np.log(M)
        
        # compute loss
        loss = torch.sum(ker(tmp,tmp))/M/M-2*torch.sum(weights@ker(seqMC,tmp))/M+lamda*torch.mean((tmp-last_x)**2)

        # backpropagation
        loss.backward()
        optimizer.step()

        if iteration == max_iter or np.abs(last_loss - loss.item()) < 1e-7:
...
for t in range(T+1):
    ...
    if t != T:
        recon_x = op(recon_x,noise=False)
print('RMSE',RMSE)
np.save('ATPFgaussker_T=10.npy',pred_x)