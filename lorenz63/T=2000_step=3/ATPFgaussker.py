...
for t in range(T+1): # train transformation[t]
    ...
    max_iter = 150
    optimizer = torch.optim.RMSprop(transformation[t].parameters(),lr=0.00008)
...
plt.title('KATPF(Gauss), T = '+str(T)+', step = '+str(t_max)+', lam = '+str(lamda))
plt.savefig('ATPFgaussker (pri T='+str(T)+' step='+str(t_max)+' lam='+str(lamda)+').png')