...
delta_reconx = 0.04 # inflaction coeff. of ATPF
eps = 1e-6
lamda = 0.0
...
class generator(nn.Module):
    def __init__(self,indim=dim+dim//2,middim=256,outdim=dim):
        ...
        self.nets = nn.Sequential(nn.Linear(indim,middim),
                                  ...
                                  nn.Tanh(),
                                  nn.Linear(middim*4,middim*4),
                                  nn.Tanh(),
                                  nn.Linear(middim*4,outdim))
    def forward(self,x):
        return self.nets(x)
...
for t in range(T+1): # train transformation[t]
    ...
    optimizer = torch.optim.Adam(transformation[t].parameters(),lr=0.001)
...
for t in range(T+1):
    ...
    if t != T:
        recon_x = op(recon_x,noise=False)
print('RMSE',RMSE)
np.save('ATPFlinker_T=100.npy',pred_x)
