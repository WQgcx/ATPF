...
C = 0
D = 1
tau = 0.018 # inflation coeff. of MPF
delta_reconx = 0.1 # inflaction coeff. of ATPF
eps = 1e-6
lamda = 0.0
...
for t in range(T+1): # train transformation[t]
    ...
    optimizer = torch.optim.Adam(transformation[t].parameters(),lr=0.0008)
...
for t in range(T+1):
    ...
    if t != T:
        recon_x = op(recon_x,noise=False)
print('RMSE',RMSE)
np.save('ATPFgaussker_T=100.npy',pred_x)