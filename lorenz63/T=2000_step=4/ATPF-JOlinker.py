...
##        Ct = (recon_x-torch.mean(recon_x,axis=0)).T@(recon_x-torch.mean(recon_x,axis=0))/(len(recon_x)-1)
##        Kt = (Ct@torch.linalg.inv(Ct+D**2*torch.eye(dim).to(device))).reshape(dim**2)
##        Kt = torch.stack(tuple(Kt for i in range(M)))
    ...
    del recon_x,seqMC,weights,last_x,yt,cov,eigvalues,eigvec
...
plt.title('KATPF-JO(linear), T = '+str(T)+', step=4, lam = '+str(lamda))
plt.savefig('ATPF-JOlinker (pri T='+str(T)+' step=4 lam='+str(lamda)+').png')
